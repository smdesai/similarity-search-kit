...
public class NativeEmbeddings: EmbeddingsProtocol {
    ...
    public func encode(sentence: [String]) async -> [[Float]]? {
        ...
        for text in sentence {
            guard let vector = await model.vector(for: text) else {
                return nil
            }
    ...
    public class func encodeSparse(sentences: [String]) -> [[(index: Int, value: Double)]]? {
        ...
        let tokenizedSentences = sentences.map {
            tokenize(sentence: $0, wordEmbedding: wordEmbedding)
        }
        ...
        let avgDocLength =
            Double(tokenizedSentences.reduce(0) { $0 + $1.count })
            / Double(tokenizedSentences.count)
        let embeddings = computeBM25(
            tokenizedSentences: tokenizedSentences, vocabulary: vocabulary, idf: idf,
            avgDocLength: avgDocLength, k1: k1, b: b)
        return embeddings
    }
    ...
    class func tokenize(sentence: String, wordEmbedding _: NLEmbedding) -> [String] {
        ...
        return tokenizer.tokens(for: sentence.startIndex ..< sentence.endIndex).map {
            String(sentence[$0])
        }
    }
    ...
    class func computeBM25(
        tokenizedSentences: [[String]], vocabulary: [String], idf: [String: Double],
        avgDocLength: Double, k1: Double, b: Double
