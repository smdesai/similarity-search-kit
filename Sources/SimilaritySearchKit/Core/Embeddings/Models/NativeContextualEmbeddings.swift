    ...
    public class NativeContextualEmbeddings: EmbeddingsProtocol {
        ...
        public actor ModelActor {
            ...
            func vector(for sentence: String) -> [Float]? {
                ...
                embedding.enumerateTokenVectors(in: sentence.startIndex ..< sentence.endIndex) {
                    (embedding, _) -> Bool in
                    for tokenEmbeddingIndex in 0 ..< embedding.count {
                        meanPooledEmbeddings[tokenEmbeddingIndex] += Float(
                            embedding[tokenEmbeddingIndex])
                    }
                    return true
                }
        ...
        public func encode(sentence: [String]) async -> [[Float]]? {
            var result: [[Float]] = []
            for s in sentence {
                if let v = await model.vector(for: s) {
                    result.append(v)
                }
            }
            return result
